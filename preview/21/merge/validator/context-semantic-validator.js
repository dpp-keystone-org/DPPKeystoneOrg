...
import { isDate, isDateTime, isDecimal, isInteger, isCountryCode, isURI, hasControlCharacters } from '../util/js/common/validation/ontology-validator.js?v=1778260823831';
 ...
 */
export async function validateContextAwarePayload(dppData) {
    const errors = [];

    let expandedData;
    try {
        expandedData = await _expand(dppData, { documentLoader: customDocumentLoader });
    } catch (e) {
        errors.push({
            instancePath: '/',
            schemaPath: '',
    ...
    function traverse(obj, path, currentIri) {
        if (Array.isArray(obj)) {
            obj.forEach((item, index) => {
                ...
                traverse(item, arrayPath || `[${index}]`, currentIri);
            });
