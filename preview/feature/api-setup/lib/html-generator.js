...
export async function generateHTML(dppJson, optionsOrCssUrl) {
  ...
  if (includeSchema) {
    try {
      ...
      const { transformDpp } = await import('../util/js/client/dpp-schema-adapter.js?v=1790088520762');
  ...
  try {
    const { loadOntology } = await import('./ontology-loader.js?v=1790088520762');
    let sector = 'dpp';
