...
test.describe('DPP Validator', () => {
  ...
  test('Ontology metadata strictly enforces xsd:date strings in the UI', async ({ page }) => {
    const invalidOntologyDpp = {
      ...
      "lastUpdate": "2024-01-01T00:00:00Z",
      "economicOperatorId": "urn:uuid:org1",
      "contentSpecificationIds": ["draft_construction_specification_id"],
      "manufacturer": {
        "organizationName": "Test Org",
        "address": {
          "streetAddress": "1 Way",
      ...
      // Intentional format violation specifically triggering semantic boundaries
      // versionDate is lenient in schema but strictly xsd:date in the construction ontology
      "versionDate": "bad-date-format"
    };

    await page.locator('#json-input').fill(JSON.stringify(invalidOntologyDpp));
    await page.locator('#validate-btn').click();
