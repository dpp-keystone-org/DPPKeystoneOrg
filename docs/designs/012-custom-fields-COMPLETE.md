...
- **[PENDING] 5p. Custom Field Implementation:**
  ...
  - **[COMPLETED] 5p-5. Namespace Collision Avoidance:** Validate that the custom field key does not conflict with existing fields in the loaded schema/ontology.
    ...
    - **[COMPLETED] 5p-5b. [Implementation] Implement Collision Logic:** Create an async helper function `getConflictingSectors(key)` in `wizard.js` that checks `coreSchema` and **all available** sector schemas (loading them on demand). Pass this function into `createVoluntaryFieldRow` in `form-builder.js` and update the validation logic to handle the async check.
  - **[DEFERRED] 5p-6. JSON Generation Integration:** Moved to a separate design document dedicated to JSON generation auditing and improvements.   - **[PENDING] 5p-6a. [Test] Add Failing Test for Typed JSON Generation:** Assert that the generated JSON reflects the correct types (Number, Boolean). Pause and allow the user to test.
    - **[DEFERRED] 5p-6b. [Implementation] Update Generator for Primitives:** Update `dpp-generator.js` to parse values based on the selected type. Pause and allow the user to test.
    - **[DEFERRED] 5p-6c. [Test] Add Failing Test for Complex JSON Generation:** Assert that Numbers with units (QuantitativeValues) and nested Groups are structured correctly in the JSON. Pause and allow the user to test.
    - **[DEFERRED] 5p-6d. [Implementation] Update Generator for Complex Types:** Update `dpp-generator.js` to handle composite and nested structures. Pause and allow the user to test.
  - **[IN PROGRESS] 5p-7. Complex Type Support (e.g., Organization):** Extend the custom field logic to support predefined complex types defined by JSON schemas (e.g., Organization), using a registry pattern.
    - **[COMPLETED] 5p-7a. [Test] Add Failing Test for Complex Type Registry:** Create a test asserting that the "Type" dropdown includes "Organization" when passed a registry.
    - **[COMPLETED] 5p-7b. [Implementation] Implement Complex Type Registry & Schema Loading:** Update `wizard.js` to pass a registry of types to `form-builder.js`. Update `createVoluntaryFieldRow` to handle async schema loading for these types.
    - **[COMPLETED] 5p-7c. [Test] Add Failing Test for Auto-Population:** Create a test that mocks a schema loader and asserts that selecting a complex type populates the group container with fields mapped from the schema (e.g., string -> Text, number -> Number).
    - **[COMPLETED] 5p-7d. [Implementation] Implement Auto-Population:** Update `form-builder.js` to populate the "Group" container with fields derived from the loaded schema (recursively).
